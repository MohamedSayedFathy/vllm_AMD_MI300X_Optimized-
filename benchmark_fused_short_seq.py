...
Compares 4 kernel modes across sequence lengths and batch sizes:
  ...
  4. mfma4_all_512  - mfma4 for all GQA + 512-token partition (seq <= 512 fused)
...
Usage:
    # Run all 4 modes (default)
    python benchmark_fused_short_seq.py

    # Run specific modes only
    python benchmark_fused_short_seq.py --modes baseline fused
    ...
    python benchmark_fused_short_seq.py --load ../benchmark_results/results_20260214.json
"""
...
Examples:
  ...
  # Compare just baseline vs fused
  python benchmark_fused_short_seq.py --modes baseline fused

  # Quick test with specific seq lengths
  python benchmark_fused_short_seq.py --seq-lengths 128 256 512

  # Full sweep with markdown output
  python benchmark_fused_short_seq.py \\
      --seq-lengths 32 64 128 256 384 512 768 1024 \\
      --batch-sizes 8 16 32 64 --markdown
  ...
  python benchmark_fused_short_seq.py --load ../benchmark_results/results.json
    ...
    # Load mode: just display saved results
    if args.load:
